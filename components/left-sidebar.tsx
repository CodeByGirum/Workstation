  ...
  return (
    <div>
      <Tooltip>
        ...
        <TooltipContent side="right" align="start" className="text-xs px-2 py-1">
          <p>{isOpen ? "Collapse" : "Expand"} section</p>
        </TooltipContent>
      </Tooltip>
      <div
        ...
        <div className="flex items-start justify-between p-4 border-b">
          ...
          <div className="flex items-center flex-shrink-0 pl-2">
            {onToggleCollapse && !isCollapsed && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <button onClick={onToggleCollapse} className="p-1.5 rounded-md hover:bg-accent hidden lg:block">
            ...
            <div className="space-y-2">
              {Object.entries(schema).map(([tableName, tableDetails]) => (
                <CollapsibleSection
                  key={tableName}
                  title={tableName}
                  icon={<tableDetails.icon size={16} />}
                  defaultOpen
                >
